...
import { generateWorld, getSpawnPositions, TERRAIN } from '../services/worldGen.js';
...
describe('World Generation', () => {
  ...
  describe('generateWorld', () => {
    ...
    it('should generate different output for different seeds', () => {
      ...
      const differences = world1.filter((tile: any, i: number) => tile.terrainType !== world2[i].terrainType);
    ...
    it('should assign resources based on terrain', () => {
      ...
      world.forEach((tile: any) => {
        const terrain = Object.values(TERRAIN).find((t: any) => t.type === tile.terrainType);
        if (terrain) {
          expect(tile.resourceType).toBe((terrain as any).resource);
    ...
    it('should place pre-built locations (mine, farm, market, town_hall)', () => {
      const world = generateWorld(SEED);
      const structures = world.filter((tile: any) => tile.structureType);

      expect(structures.length).toBeGreaterThanOrEqual(5); // At least 5 structures (2 mine, 2 farm, 1 market, 1 town_hall)

      const structureTypes = structures.map((s: any) => s.structureType);
