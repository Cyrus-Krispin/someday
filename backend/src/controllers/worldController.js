...
const createWorld = async (req, res) => {
  try {
    ...
    const world = await worldService.createWorld(seed);
...
const joinWorld = async (req, res) => {
  ...
  try {
    ...
    const world = await worldService.getWorldByJoinCode(joinCode.toUpperCase());
    ...
    const spawnPositions = await worldService.getSpawnPositionsForWorld(world.id, playerCount + 1);
...
const getWorldState = async (req, res) => {
  ...
  try {
    ...
    const tiles = await worldService.getWorldTiles(worldId, xMin, yMin, xMax, yMax);
...
module.exports = { createWorld, joinWorld, getWorldState };
