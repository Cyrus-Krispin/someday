 ...
 */
const getTurnState = async (worldId) => {
 ...
 */
const processTurn = async (worldId, playerId, actions) => {
  ...
  try {
    ...
    for (const action of actions) {
      switch (action.type) {
        ...
        case 'gather':
          const gatherResult = await processGather(client, worldId, playerId, player, gatherResult?.tile);
          // Update resources handled inside
          break;

        case 'work':
          const workResult = await processWork(client, worldId, playerId, player, action);
          // Tokens/score updated inside
          break;
      }
    }
 ...
 */
const processMove = async (client, worldId, player, action, movementRemaining) => {
 ...
 */
const processGather = async (client, worldId, playerId, player) => {
  // Get current tile
  const tileResult = await client.query(
    'SELECT * FROM tiles WHERE world_id = $1 AND x = $2 AND y = $3',
 ...
 */
const processWork = async (client, worldId, playerId, player, action) => {
 ...
 */
const endTurn = async (worldId, playerId) => {
