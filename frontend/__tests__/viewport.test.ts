...
describe('viewport Utils', () => {
  describe('calculateViewport', () => {
    it('should calculate 15x15 viewport around player', () => {
      ...
      expect(viewport.xMin).toBe(8); // 15 - 7
      expect(viewport.xMax).toBe(22); // 15 + 7
    ...
    it('should handle player near left edge', () => {
      ...
      expect(viewport.xMin).toBe(0); // Clamped to 0
      expect(viewport.xMax).toBe(14); // 2 + 12 (adjusted)
    });
    ...
    it('should handle player near right edge', () => {
      ...
      expect(viewport.xMax).toBe(29); // Clamped to mapSize - 1
      expect(viewport.xMin).toBe(15); // mapSize - 1 - 2*radius (preserves 15-wide viewport)
    });
    ...
    it('should handle player near top edge', () => {
      ...
      expect(viewport.yMin).toBe(0); // Clamped to 0
    ...
    it('should handle player near bottom edge', () => {
      ...
      expect(viewport.yMax).toBe(29); // Clamped to mapSize - 1
      expect(viewport.yMin).toBe(15); // mapSize - 1 - 2*radius (preserves 15-tall viewport)
    });
  });
});
